...
const buyItem_action = async (
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_REST_URL}/cosmetics/buy/${uuid}`, {
    method: "POST",
    headers: {
  ...
  return JSON.stringify({ status: 1, message: language.shop.toast.buy_success });
};

export { buyItem_action };
