...
export interface UnselectBonusDataType {
  entityUuid: string,
  type: 'strategy' | 'duel',
}
const unselectBonus_action = async (
  data: UnselectBonusDataType,
): Promise<string> => {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_REST_URL}/bonus/unselect`, {
    method: "POST",
